...
var hatch_specs = {
	z_opening_ft: 6.0,
	z_floor_ft: -14,	#  z-axis offset for walker exit from aircraft origin
...
# TODO: remove listener on tied property heading-offset-deg!
...
var get_out = func (loc) {
	...
	if (c_view == 0) {	# remember point of exit
		...
		setprop("sim/view[201]/enabled",1);
		setprop("sim/view[202]/enabled",1);
...
setprop("sim/current-view/view-number", 0);
	...
	parachute_deployed_sec = 0;
	setprop("sim/walker/parachute-opened-sec", 0);
	setprop("sim/view[201]/enabled", 0);
	setprop("sim/view[202]/enabled", 0);
}
