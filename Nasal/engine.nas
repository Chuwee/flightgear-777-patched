...
var Engine = {
    new : func(eng_num){
        ...
        m.eng_num = eng_num;
        m.eng = props.globals.getNode("engines/engine["~eng_num~"]");
		...
		m.out_of_fuel = props.globals.getNode("engines/engine["~eng_num~"]/out-of-fuel",1);
		m.out_of_fuel.setBoolValue(1);
    ...
    update : func {
        ...
        if(getprop("sim/flight-model") == "yasim")
        {
            if(me.fuel_out.getBoolValue() or me.cutoff.getBoolValue())
            {
				...
				me.fuel_gph.setDoubleValue(0);
                me.running.setBoolValue(0);
                me.egt.setDoubleValue(me.egt_degf.getValue());
            } elsif (me.running.getBoolValue() and !me.cutoff.getBoolValue()) {
				me.out_of_fuel.setBoolValue(0);
			}
