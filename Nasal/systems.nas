...
{
    aircraft.livery.init("Aircraft/777/Models/Liveries-300");
}
else if(vmodel == "-300ER")
{
    aircraft.livery.init("Aircraft/777/Models/Liveries-300ER");
}
else if(vmodel == "-200")
{
    aircraft.livery.init("Aircraft/777/Models/Liveries-200");
}
else if(vmodel == "-200ER")
{
    aircraft.livery.init("Aircraft/777/Models/Liveries-200ER");
}
else if(vmodel == "-200LR")
{
    aircraft.livery.init("Aircraft/777/Models/Liveries-200LR");
}


# This version check warns the user about FG version being too low, 
# but does not prevent users from still using it.
var versionCheck = func(){
	...
	for (var i = 0; i < size(fgfsMin); i += 1) {
        if (fgfsVer[i] > fgfsMin[i]) return 1;
        if (fgfsVer[i] < fgfsMin[i]) return 0;
    }
...
if(!versionCheck()) settimer(func(){
    setprop("sim/messages/copilot", 
            "Warning: Your FlightGear version is below the required minimum "
            ~ getprop("/sim/minimum-flightgear-version")
            ~ " for this aircraft. You may continue using this aircraft," 
...
var DemoLandingHoldTargets = func {
    var hold_hdg = getprop("orientation/heading-deg");
    if (hold_hdg == nil) hold_hdg = getprop("sim/presets/heading-deg");
    if (hold_hdg == nil) hold_hdg = 322;

    var hold_alt = getprop("instrumentation/altimeter/indicated-altitude-ft");
    if (hold_alt == nil) hold_alt = getprop("sim/presets/altitude-ft");
    if (hold_alt == nil) hold_alt = 4500;
    hold_alt = int(hold_alt / 100 + 0.5) * 100;

    var hold_spd = getprop("sim/presets/airspeed-kt");
    if (hold_spd == nil) hold_spd = getprop("instrumentation/airspeed-indicator/indicated-speed-kt");
    if (hold_spd == nil) hold_spd = 170;
    hold_spd = max(150, min(175, hold_spd));

    setprop("sim/model/demo-ap/enabled", 1);
    setprop("sim/model/demo-ap/engage-ap", 1);
    setprop("sim/model/demo-ap/target-heading-deg", hold_hdg);
    setprop("sim/model/demo-ap/target-altitude-ft", hold_alt);
    setprop("sim/model/demo-ap/target-speed-kt", hold_spd);
    setprop("autopilot/settings/heading-bug-deg", hold_hdg);
    setprop("autopilot/settings/counter-set-altitude-ft", hold_alt);
    setprop("autopilot/settings/actual-target-altitude-ft", hold_alt);
    setprop("autopilot/settings/target-speed-kt", hold_spd);
}
...
var start_updates = func {
    if (getprop("position/gear-agl-ft")>30)
    {
        # airborne startup
        Startup();
        var demo_landing = getprop("sim/model/demo-landing-ready");
...
var DemoLandingAudioSync = func {
    if (!getprop("sim/model/demo-landing-audio-sync")) {
        return;
    }
    var n1_0 = getprop("engines/engine[0]/n1") or 0;
    var n1_1 = getprop("engines/engine[1]/n1") or 0;
    var n2_0 = getprop("engines/engine[0]/n2") or 0;
    var n2_1 = getprop("engines/engine[1]/n2") or 0;
    if (getprop("engines/engine[0]/run")) {
        setprop("engines/engine[0]/rpm", max(n1_0, 20));
        setprop("engines/engine[0]/n2rpm", max(n2_0, 70));
    }
    if (getprop("engines/engine[1]/run")) {
        setprop("engines/engine[1]/rpm", max(n1_1, 20));
        setprop("engines/engine[1]/n2rpm", max(n2_1, 70));
    }
    settimer(DemoLandingAudioSync, 0.2);
}

var DemoAirStartAudioSync = func {
    if (!getprop("sim/model/demo-air-start-audio-sync")) {
        return;
    }
    var n1_0 = getprop("engines/engine[0]/n1") or 0;
    var n1_1 = getprop("engines/engine[1]/n1") or 0;
    ...
    settimer(DemoAirStartAudioSync, 0.2);
}

var DemoLandingReady = func {
    Startup();
    var DemoLandingReadyEnd = maketimer(1.5, func() {
        setprop("engines/engine[0]/run", 1);
        setprop("engines/engine[1]/run", 1);
        setprop("engines/engine[0]/rpm", 22);
        setprop("engines/engine[1]/rpm", 22);
        setprop("engines/engine[0]/n2rpm", 72);
        setprop("engines/engine[1]/n2rpm", 72);
        setprop("controls/engines/engine[0]/cutoff", 0);
        setprop("controls/engines/engine[1]/cutoff", 0);
        setprop("engines/engine[0]/out-of-fuel", 0);
        setprop("engines/engine[1]/out-of-fuel", 0);
        setprop("controls/engines/engine[0]/throttle-act", 0.45);
        setprop("controls/engines/engine[1]/throttle-act", 0.45);
        setprop("controls/engines/engine[0]/throttle", 0.45);
        setprop("controls/engines/engine[1]/throttle", 0.45);
        setprop("controls/engines/engine[0]/thrust-lever", 0.45);
        setprop("controls/engines/engine[1]/thrust-lever", 0.45);
        ...
        setprop("instrumentation/afds/inputs/at-armed[1]", 1);
        DemoLandingHoldTargets();
        setprop("sim/model/demo-landing-audio-sync", 1);
        settimer(DemoLandingAudioSync, 0.1);
        setprop("sim/model/demo-landing-ready", 0);
        DemoLandingReadyEnd.stop();
    });
    DemoLandingReadyEnd.start();
}

var DemoAirStartApply = func {
    Startup();

    var throttle = getprop("sim/model/demo-air-start/throttle");
    if (throttle == nil) throttle = 0.50;

    var flaps = getprop("sim/model/demo-air-start/flaps");
    if (flaps == nil) flaps = 0;

    var hold_hdg = getprop("sim/model/demo-air-start/target-heading-deg");
    if (hold_hdg == nil) hold_hdg = getprop("sim/presets/heading-deg");
    if (hold_hdg == nil) hold_hdg = getprop("orientation/heading-deg");
    if (hold_hdg == nil) hold_hdg = 322;

    var hold_alt = getprop("sim/model/demo-air-start/target-altitude-ft");
    if (hold_alt == nil) hold_alt = getprop("sim/presets/altitude-ft");
    if (hold_alt == nil) hold_alt = getprop("instrumentation/altimeter/indicated-altitude-ft");
    if (hold_alt == nil) hold_alt = 4500;
    hold_alt = int(hold_alt / 100 + 0.5) * 100;

    var hold_spd = getprop("sim/model/demo-air-start/target-speed-kt");
    if (hold_spd == nil) hold_spd = getprop("sim/presets/airspeed-kt");
    if (hold_spd == nil) hold_spd = getprop("instrumentation/airspeed-indicator/indicated-speed-kt");
    if (hold_spd == nil) hold_spd = 170;
    hold_spd = max(150, min(220, hold_spd));
    ...
    setprop("controls/engines/engine[0]/throttle-act", throttle);
    setprop("controls/engines/engine[1]/throttle-act", throttle);
    setprop("controls/engines/engine[0]/throttle", throttle);
    setprop("controls/engines/engine[1]/throttle", throttle);
    setprop("controls/engines/engine[0]/thrust-lever", throttle);
    setprop("controls/engines/engine[1]/thrust-lever", throttle);
    ...
    setprop("controls/flight/speedbrake", 0);
    setprop("controls/flight/elevator-trim", 0.125);
    setprop("sim/model/demo-air-start-audio-sync", 1);
}

var DemoAirStartEngageAP = func {
    var hold_hdg = getprop("sim/model/demo-air-start/target-heading-deg");
    if (hold_hdg == nil) hold_hdg = getprop("sim/presets/heading-deg");
    if (hold_hdg == nil) hold_hdg = getprop("orientation/heading-deg");
    if (hold_hdg == nil) hold_hdg = 322;
...
var DemoAirStartReady = func {
    var repetitions = 0;
    var DemoAirStartTimer = maketimer(0.5, func() {
        repetitions += 1;
        DemoAirStartApply();

        if (repetitions >= 12) {
            setprop("sim/model/demo-air-start-ready", 0);
            DemoAirStartEngageAP();
            settimer(DemoAirStartAudioSync, 0.1);
            DemoAirStartTimer.stop();
        }
    });
    DemoAirStartTimer.start();
}

setlistener("sim/model/demo-landing-ready", func(node) {
    if (node.getValue()) {
        DemoLandingReady();
    }
}, 1, 0);

setlistener("sim/model/demo-air-start-ready", func(node) {
    if (node.getValue()) {
        DemoAirStartReady();
    }
}, 1, 0);
...
controls.gearDown = func(v) {
    # Pseudopilot patch: removed the gear[1]/wow guard on retraction
    # so gear-up commanded from external sources (PIEM takeoff handoff)
    # is not silently dropped while the WOW sensor is still settling.
    if (v < 0) {
