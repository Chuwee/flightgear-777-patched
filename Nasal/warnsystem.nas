...
# 2010, Thorsten Brehm
# 2026, upgraded bij Isaak Dieleman
#
# This file is licensed under the GNU GPLv2 license as part of the FlightGear Boeing 777. See "License" for more information about what you can and cannot do.
#
...
var WEU =
{
    new : func(prop1)
    {
        ...
        m.node_fuel_temp     = props.globals.getNode("engines/engine[0]/fuel-temperature-degc");
        ...
        m.fuel_c_pump2	= 0;
        m.fuel_c_qty	= 0;
        m.fuel_jettnozzleL = 0;
        m.fuel_jettnozzleM0 = 0;
        m.fuel_jettnozzleM2 = 0;
        m.fuel_jettnozzleR = 0;
		
		m.fuel_cutoff_switch_L = 0;
		m.fuel_cutoff_switch_R = 0;
		...
		m.Hydr_LEDP_lowpress = 0;
		m.Hydr_LACMP_lowpress = 0;
		m.Hydr_C1ACMP_lowpress = 0;
		m.Hydr_C1ADP_lowpress = 0;
		m.Hydr_C2ACMP_lowpress = 0;
		m.Hydr_C2ADP_lowpress = 0;
		m.Hydr_REDP_lowpress = 0;
		m.Hydr_RACMP_lowpress = 0;
		...
		m.HYDR_REDP_ovht = 0;
		m.HYDR_RACMP_ovht = 0;
		m.HYDR_C1ACMP_ovht = 0;
		m.HYDR_C1ADP_ovht = 0;
		m.HYDR_C2ACMP_ovht = 0;
		m.HYDR_C2ADP_ovht = 0;
        ...
        setlistener("controls/gear/brake-parking-set",      func { Weu.update_listener_inputs() } );
		...
		setlistener("engines/engine[0]/run", func { Weu.update_listener_inputs() } );
		setlistener("engines/engine[1]/run", func { Weu.update_listener_inputs() } );
        ...
        setlistener("controls/fuel/tank[0]/nozzle-valve", func { Weu.update_listener_inputs() } );
        setlistener("controls/fuel/tank[0]/boost-pump[2]", func { Weu.update_listener_inputs() } );
        setlistener("controls/fuel/tank[2]/boost-pump[2]", func { Weu.update_listener_inputs() } );
        setlistener("controls/fuel/tank[2]/nozzle-valve", func { Weu.update_listener_inputs() } );
		
		# Wow
		setlistener("gear/gear[0]/wow", func { Weu.update_listener_inputs() } );
		setlistener("gear/gear[1]/wow", func { Weu.update_listener_inputs() } );
		setlistener("gear/gear[2]/wow", func { Weu.update_listener_inputs() } );

		# Hydraulics
		setlistener("systems/hydraulics/system[0]/m-pump[0]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[0]/m-pump[1]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[1]/m-pump[0]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[1]/m-pump[1]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[1]/m-pump[2]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[1]/m-pump[3]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[1]/m-pump[4]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[2]/m-pump[0]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/system[2]/m-pump[1]/press-low", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/warnsystem/PressSysLeft", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/warnsystem/PressSysCenter", func { Weu.update_listener_inputs() } );
		setlistener("systems/hydraulics/warnsystem/PressSysRight", func { Weu.update_listener_inputs() } );
...
## so the advisory section below is not quite accurate
	...
	if (me.fuel_temp < -36)
	    append(me.msgs_advisory," FUEL TEMP LOW");
    if (me.fuel_jettnozzleL == 2)
        append(me.msgs_advisory," FUEL JETT NOZZLE L");
    if (me.fuel_jettnozzleR == 2)
        append(me.msgs_advisory," FUEL JETT NOZZLE R");
    if ((me.fuel_jettnozzleM0 == 2) or (me. fuel_jettnozzleM2 == 2)) {
        if (getprop("controls/fuel/tank[1]/nozzle-valve[0]") != 2)
        append(me.msgs_advisory," FUEL JETT NOZZLE MAIN");
    }
	
	# Caution and advisory messages for hydraulic system
		# Hydr System Pressure drops
	if (me.Hydr_PressSysL and me.Hydr_PressSysC and me.Hydr_PressSysR) {
		append(me.msgs_caution,"HYD PRESS SYS L+C+R");
	} elsif (me.Hydr_PressSysL and me.Hydr_PressSysR) {
		append(me.msgs_caution,"HYD PRESS SYS L+R");
	} elsif (me.Hydr_PressSysL and me.Hydr_PressSysC) {
		append(me.msgs_caution,"HYD PRESS SYS L+C");
	} elsif (me.Hydr_PressSysC and me.Hydr_PressSysR) {
		append(me.msgs_caution,"HYD PRESS SYS R+C");
	} elsif (me.Hydr_PressSysL) {
		append(me.msgs_caution,"HYD PRESS SYS L");
	} elsif (me.Hydr_PressSysR) {
		append(me.msgs_caution,"HYD PRESS SYS R");
	} elsif (me.Hydr_PressSysC) {
		append(me.msgs_caution,"HYD PRESS SYS C");
	}
	
		# Hydr Primary Pumps Pressure drops
	if (!me.Hydr_PressSysL and me.Hydr_LEDP_lowpress) {
		append(me.msgs_advisory," HYD PRESS PRI L");
	}	
	if (!me.Hydr_PressSysR and me.Hydr_REDP_lowpress) {
		append(me.msgs_advisory," HYD PRESS PRI R");
	}	
	if (!me.Hydr_PressSysC and me.Hydr_C1ACMP_lowpress) {
		append(me.msgs_advisory," HYD PRESS PRI C1");
	}
	if (!me.Hydr_PressSysC and me.Hydr_C2ACMP_lowpress) {
		append(me.msgs_advisory," HYD PRESS PRI C2");
	}
	
		# Hydr Demand Pumps Pressure drops
	if (!me.Hydr_PressSysL and me.Hydr_LACMP_lowpress) {
		append(me.msgs_advisory," HYD PRESS DEM L");
	}
	if (!me.Hydr_PressSysR and me.Hydr_RACMP_lowpress) {
		append(me.msgs_advisory," HYD PRESS DEM R");
	}
	if (!me.Hydr_PressSysC and me.Hydr_C1ADP_lowpress) {
		append(me.msgs_advisory," HYD PRESS DEM C1");
	}
	if (!me.Hydr_PressSysC and me.Hydr_C2ADP_lowpress) {
		append(me.msgs_advisory," HYD PRESS DEM C2");
	}
	# }
		
		# Hydraulics Overheat
	if (me.HYDR_LEDP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT PRI L");		
	}
	if (me.HYDR_REDP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT PRI R");		
	}
	if (me.HYDR_C1ACMP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT PRI C1");		
	}
	if (me.HYDR_C2ACMP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT PRI C2");		
	}
	
	if (me.HYDR_LACMP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT DEM L");		
	}
	if (me.HYDR_RACMP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT DEM R");		
	}
	if (me.HYDR_C1ADP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT DEM C1");		
	}
	if (me.HYDR_C2ADP_ovht) {
		append(me.msgs_advisory," HYD OVERHEAT DEM C2");		
	}

		# Hydraulic reservoirs low
	if (me.HYDR_L_qty <= 0.4) {
		append(me.msgs_advisory," HYD QTY LOW L");		
	}
	if (me.HYDR_C_qty <= 0.4) {
		append(me.msgs_advisory," HYD QTY LOW C");		
	}
	if (me.HYDR_R_qty <= 0.4) {
		append(me.msgs_advisory," HYD QTY LOW R");		
	}
...
#### stall warnings and other sounds ####
    update_sounds : func
    {
        ...
        # Why the hell are these calculations in update_sounds??? 
        # Definitely needs refactoring. At least renaming. - LIANG Sidi, 20250822 Agree :D Isaak Dieleman, 20251227
	    ...
	    me.siren.setBoolValue(0);
            ...
            if (me.fuel_cutoff_switch_L or me.fuel_cutoff_switch_R) { # Master Caution is inhibited when the aicraft is shut down.
...
#### update listener inputs ####
    update_listener_inputs : func()
    {
        ...
        me.wow = getprop("gear/gear[0]/wow") and getprop("gear/gear[1]/wow") and getprop("gear/gear[2]/wow");
		me.wowMain = getprop("gear/gear[1]/wow") and getprop("gear/gear[2]/wow");
        ...
        me.fuel_jettnozzleL = getprop("controls/fuel/tank[0]/nozzle-valve");
        me.fuel_jettnozzleM0 = getprop("controls/fuel/tank[0]/boost-pump[2]");
        me.fuel_jettnozzleM2 = getprop("controls/fuel/tank[2]/boost-pump[2]");
        me.fuel_jettnozzleR = getprop("controls/fuel/tank[2]/nozzle-valve");
		me.fuel_cutoff_switch_L = getprop("controls/engines/engine[0]/cutoff-switch");
		me.fuel_cutoff_switch_R = getprop("controls/engines/engine[1]/cutoff-switch");
		...
		me.engl_running = getprop("engines/engine[0]/run");
		me.engr_running = getprop("engines/engine[0]/run");
		
		me.Hydr_LEDP_lowpress = getprop("systems/hydraulics/system[0]/m-pump[0]/press-low");
		me.Hydr_LACMP_lowpress = getprop("systems/hydraulics/system[0]/m-pump[1]/press-low");
		me.Hydr_C1ACMP_lowpress = getprop("systems/hydraulics/system[1]/m-pump[0]/press-low");
		me.Hydr_C1ADP_lowpress = getprop("systems/hydraulics/system[1]/m-pump[1]/press-low");
		me.Hydr_C2ACMP_lowpress = getprop("systems/hydraulics/system[1]/m-pump[2]/press-low");
		me.Hydr_C2ADP_lowpress = getprop("systems/hydraulics/system[1]/m-pump[3]/press-low");
		me.Hydr_REDP_lowpress = getprop("systems/hydraulics/system[2]/m-pump[0]/press-low");
		me.Hydr_RACMP_lowpress = getprop("systems/hydraulics/system[2]/m-pump[1]/press-low");
		
		me.Hydr_PressSysL = getprop("systems/hydraulics/warnsystem/PressSysLeft");
		me.Hydr_PressSysC = getprop("systems/hydraulics/warnsystem/PressSysCenter");
		me.Hydr_PressSysR = getprop("systems/hydraulics/warnsystem/PressSysRight");
		
		me.HYDR_LEDP_ovht = getprop("systems/hydraulics/system[0]/m-pump[0]/overheat");
		me.HYDR_LACMP_ovht = getprop("systems/hydraulics/system[0]/m-pump[1]/overheat");
		me.HYDR_REDP_ovht = getprop("systems/hydraulics/system[2]/m-pump[0]/overheat");
		me.HYDR_RACMP_ovht = getprop("systems/hydraulics/system[2]/m-pump[1]/overheat");
		me.HYDR_C1ACMP_ovht = getprop("systems/hydraulics/system[1]/m-pump[0]/overheat");
		me.HYDR_C1ADP_ovht = getprop("systems/hydraulics/system[1]/m-pump[1]/overheat");
		me.HYDR_C2ACMP_ovht = getprop("systems/hydraulics/system[1]/m-pump[2]/overheat");
		me.HYDR_C2ADP_ovht = getprop("systems/hydraulics/system[1]/m-pump[3]/overheat");
		
		me.HYDR_L_qty = getprop("consumables/hydraulics/reservoir[0]/level-norm-cmd");
		me.HYDR_C_qty = getprop("consumables/hydraulics/reservoir[1]/level-norm-cmd");
		me.HYDR_R_qty = getprop("consumables/hydraulics/reservoir[2]/level-norm-cmd");
    },
    ...
    update_at_mode : func()
    {
		...
		if ((me.at_mode != 0)and(at_mode == 0)and !me.wowMain)
		{
            # AT has disconnected
            me.at_disconnect = 1;
		}
    ...
    #### update caution messages ####
		# Caution alert beeper and master caution lights are inhibitted with both fuel control switches in the cutoff position
	update_caution_msgs : func()
	{
		var totalno = size(me.msgs_caution);
		me.cautionno.setValue(totalno);
		if (me.cautionno.getValue() > me.slowcaution.getValue() and (me.fuel_cutoff_switch_L or me.fuel_cutoff_switch_R))
		{
...
#### main WEU update ####
    update : func()
    {
        ...
        if (me.enabled)
        {
            ...
            me.fuel_temp  = me.node_fuel_temp.getValue();
