...
var rwylights = props.globals.initNode("controls/lighting/turn-off-lights", 0, "BOOL");
...
var init_switches = func{
    ...
    append(lights_output,props.globals.initNode("systems/electrical/outputs/rwy-light[0]",0,"DOUBLE"));
    append(lights_load,1);
    append(lights_input,props.globals.initNode("controls/lighting/R-RwayLight",0,"BOOL"));
    append(lights_output,props.globals.initNode("systems/electrical/outputs/rwy-light[1]",0,"DOUBLE"));
    append(lights_load,1);
    ...
    append(rbus_output,props.globals.initNode("controls/fuel/xfeedaft-valve/powered",0,"BOOL"));
    append(rbus_load,1);
    append(rbus_input,props.globals.initNode("controls/fuel/xfeedfwd-switch",0,"BOOL"));
    append(rbus_output,props.globals.initNode("controls/fuel/xfeedfwd-valve/powered",0,"BOOL"));
    append(rbus_load,1);
...
update_virtual_bus = func(dt) {
    ...
    if(lidg.get_transition()) {
        ...
        if(external_primary.valid) {
            r_gcb.setValue(0);
            if(external_secondary.valid) r_btb.setValue(0);
            else {
                l_gcb.setValue(0);
                apb.setValue(0);
                l_btb.setValue(1);
        ...
        if(external_secondary.valid) {
            ...
            l_gcb.setValue(0);
            l_btb.setValue(1);
            if(external_primary.valid) r_btb.setValue(0);
            else r_gcb.setValue(0);
        }
    ...
    if(lidg.valid) l_main_ac.setValue(lidg.get_output_volts());
    elsif(apu.valid) l_main_ac.setValue(115);
    elsif(external_secondary.valid) l_main_ac.setValue(115);
    elsif(external_primary.valid) l_main_ac.setValue(115);
    elsif(ridg.valid) l_main_ac.setValue(ridg.get_output_volts());
    else l_main_ac.setValue(0);

    l_xfr.setValue(l_main_ac.getValue());

    if(ridg.valid) r_main_ac.setValue(ridg.get_output_volts());
    elsif(external_primary.valid) r_main_ac.setValue(115);
    elsif(external_secondary.valid) r_main_ac.setValue(115);
    elsif(apu.valid) r_main_ac.setValue(115);
    elsif(lidg.valid) r_main_ac.setValue(lidg.get_output_volts());
    else r_main_ac.setValue(0);
...
update_electrical = func {
    ...
    settimer(update_electrical, 0.05);
}
