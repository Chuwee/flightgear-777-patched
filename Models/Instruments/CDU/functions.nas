...
var getArrAirport = func(){
	var dest = getprop("/instrumentation/cdu/arr/airport");
	if (dest == nil or dest == ""){
		dest = getprop("/autopilot/route-manager/destination/airport");
	}
	return dest;
}

var echoAppr = func(page,selectedRwy = ""){
	var dest = getArrAirport();
...
var echoStars = func(page){
	...
	if(dest != ""){
		...
		var allStarsTmp = call(fetchStars, nil, var err = []);
		if(size(err)){
			allStars = [];
		}else if(allStarsTmp == nil){
			allStars = [];
		}else{
			allStars = allStarsTmp;
		}
		var echoedStars = [];
		var i = 0;
		var starsNum = size(allStars);
		var countStart = (page - 1) * 5;
		count = countStart;
		while(i <= 5){
			if(count <= starsNum-1){
				append(echoedStars, allStars[count]);
				i = i + 1;
				count = count + 1;
			}else{
				append(echoedStars, "");
...
var echoRwysAppr = func(pageRwys){
	if(getArrAirport() != ""){
		var apt = airportinfo(getArrAirport());
		var allRwys = keys(apt.runways);
		var echoedRwys = [];
...
var getArrMaxPage = func(){
	var dest = getArrAirport();
	if (dest == nil or dest == "") return 1;
	var apt = airportinfo(dest);
	if (apt == nil) return 1;
	var apprList = apt.getApproachList();
	var apprCount = 0;
	if (apprList != nil){
		apprCount = size(apprList);
	}
	var rwys = keys(apt.runways);
	var rwyCount = 0;
	if (rwys != nil){
		rwyCount = size(rwys);
	}
	...
	var apprPages = int((apprCount + 4) / 5);
	if (apprPages < 1) { apprPages = 1; }
	var remainder = apprCount - int(apprCount / 5) * 5;
	var slotsOnLastApprPage = 0;
	if (remainder != 0){
		slotsOnLastApprPage = 5 - remainder;
	}
	var runwaysLeft = rwyCount - slotsOnLastApprPage;
	if (runwaysLeft < 0) { runwaysLeft = 0; }
	var rwyPages = int((runwaysLeft + 4) / 5);
	return apprPages + rwyPages;
}

var arrNextPge = func(){
	var tmp = getprop("/instrumentation/cdu/appr/page");
	if (tmp == nil) { tmp = 1; }
	var maxPage = getArrMaxPage();
	if(tmp + 1 <= maxPage){
		tmp = tmp + 1;
...
var getLastPos = func(){
	#//Using current position as 'last position' for now. Is it necessary to implement a real 'last position'?
	var currentLatDeg = getprop("/position/latitude-deg");
	var currentLonDeg = getprop("/position/longitude-deg");
	setprop("/instrumentation/fmc/lastposlat", currentLatDeg);
	setprop("/instrumentation/fmc/lastposlon", currentLonDeg);

	var lastPosGot = latdeg2latDMM(currentLatDeg)~" "~londeg2lonDMM(currentLonDeg);
...
var execPushed = func(){
	if (getprop("/autopilot/route-manager/isArmed") == 1){
		...
		if(getprop("/autopilot/route-manager/destination/newStar") != nil){
			setprop("/autopilot/route-manager/destination/star", getprop("/autopilot/route-manager/destination/newStar"));
		}
...
var crzAltCDUInput = func(){
	...
	if(find("FL", cduInput) != -1){ #//Input form: FLxxx
		if(size(cduInput) <= 5){
            var numberPart = substr(cduInput,2,size(cduInput));
			if(num(numberPart) != nil){
                if(numberPart >= 100 and numberPart <= 412){
	...
	}else if((find("FL", cduInput) == -1) and num(cduInput) != nil){ #//Input form: pure number
		if(cduInput >= 1000 and cduInput <= 41200){
			...
			cduInput = "";
        }else if(cduInput >= 10 and cduInput <= 412){
        ...
        #//else for "(find("FL", cduInput) == -1) and num(cduInput) != nil"
        cduInput = "INVALID ENTRY";
    }
    return cduInput;
}
