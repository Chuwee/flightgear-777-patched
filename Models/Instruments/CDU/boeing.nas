# Copyright (C) 2018-2023 Sidi Liang, Yongfan Li
#
...
var cduInitialize = func(){
	...
	props.getNode("/",1).setValue("/autopilot/route-manager/cruise/altitude-FL","");
	...
	props.getNode("/",1).setValue("/instrumentation/cdu/dep/showSelOnly", 0);
	props.getNode("/",1).setValue("/instrumentation/cdu/appr/apprIsSelected", 0);
	props.getNode("/",1).setValue("/instrumentation/cdu/stars/starIsSelected", 0);
	props.getNode("/",1).setValue("/instrumentation/cdu/arr/airport", "");
	props.getNode("/",1).setValue("/instrumentation/cdu/arr/isSelectable", 1);
	props.getNode("/",1).setValue("/instrumentation/cdu/arr/showSelOnly", 0);
	...
	props.getNode("/",1).setValue("/autopilot/route-manager/isChanged",0);
	props.getNode("/",1).setValue("/instrumentation/cdu/dep/isChanged",0);
	props.getNode("/",1).setValue("/instrumentation/cdu/arr/isChanged",0);
...
#//Disable those log message as this project has been merged into the main 777 project
...
var del = func(){
	...
	setprop("/instrumentation/fmc/isArmed", -1);
}
...
var deleteWaypoint = func(k) {
	setprop("/autopilot/route-manager/input", sprintf("@DELETE%d", k + ((crtPageNum - 1) * 5)));
}

var insertWaypoint = func(k, cduInput) {
	setprop("/autopilot/route-manager/input", sprintf("@INSERT%d:%s", k + ((crtPageNum - 1) * 5), cduInput));
}

var isBlank = func(val) {
	return val == nil or val == "";
}

var getCduOutput = func(line, side) {
	return getprop("/instrumentation/cdu/output/line" ~ line ~ "/" ~ side);
}

var trimProcRunwaySuffix = func(name){
	# For solving the overlapping issue
	# Might not reflect the realistic behaviour, more investigation needed
	# LIANG Sidi, 2025.12.31
	if (isBlank(name)) return name;
	var idx = find(" RWY", name);
	if (idx < 0) { idx = find(" RW", name); }
	if (idx >= 0) return left(name, idx);
	idx = find(".", name);
	if (idx < 0) return name;
	var suffix = substr(name, idx + 1, size(name));
	var suffixLen = size(suffix);
	if (suffixLen < 2 or suffixLen > 3) return name;
	if (num(substr(suffix, 0, 2)) == nil) return name;
	if (suffixLen == 2) return left(name, idx);
	var lastChar = string.uc(substr(suffix, 2, 3));
	if (lastChar == "L" or lastChar == "R" or lastChar == "C"){
		return left(name, idx);
	}
	return name;
}

var getArrAirport = func(){
	var arrAirport = getprop("/instrumentation/cdu/arr/airport");
	if (isBlank(arrAirport)){
		arrAirport = getprop("/autopilot/route-manager/destination/airport");
	}
	return arrAirport;
}

var setArrivalContext = func(arrAirport, selectable){
	if (arrAirport == nil) { arrAirport = ""; }
	setprop("/instrumentation/cdu/arr/airport", arrAirport);
	setprop("/instrumentation/cdu/arr/isSelectable", selectable);
	setprop("/instrumentation/cdu/arr/showSelOnly", 0);
	setprop("/instrumentation/cdu/appr/page", 1);
	var arrDirty = getprop("/instrumentation/cdu/arr/isChanged");
	if (arrDirty == nil) { arrDirty = 0; }
	if (arrDirty == 0 and selectable != 1){
		setprop("/instrumentation/cdu/appr/apprIsSelected", 0);
		setprop("/instrumentation/cdu/stars/starIsSelected", 0);
	}
}

var updateRouteChangedFlag = func(){
	var depChanged = getprop("/instrumentation/cdu/dep/isChanged");
	var arrChanged = getprop("/instrumentation/cdu/arr/isChanged");
	if (depChanged == nil) { depChanged = 0; }
	if (arrChanged == nil) { arrChanged = 0; }
	if (depChanged != 0 or arrChanged != 0){
...
var markDepartureDirty = func(){
	setprop("/instrumentation/cdu/dep/isChanged",1);
	setprop("/autopilot/route-manager/isArmed",1);
	updateRouteChangedFlag();
}

var markArrivalDirty = func(){
	setprop("/instrumentation/cdu/arr/isChanged",1);
	setprop("/instrumentation/cdu/arr/showSelOnly",1);
	setprop("/autopilot/route-manager/isArmed",1);
	updateRouteChangedFlag();
}

var clearDepartureDirty = func(){
	setprop("/instrumentation/cdu/dep/isChanged",0);
	setprop("/instrumentation/cdu/dep/showSelOnly",0);
	updateRouteChangedFlag();
}

var clearArrivalDirty = func(){
	setprop("/instrumentation/cdu/arr/isChanged",0);
	setprop("/instrumentation/cdu/arr/showSelOnly",0);
	updateRouteChangedFlag();
}

var clearDepartureSidSelection = func(){
...
var selectDepartureSid = func(line){
	if (getprop("/instrumentation/cdu/dep/showSelOnly") == 1 and getprop("/instrumentation/cdu/dep/isChanged") != 0) return;
	var sid = getCduOutput(line, "left");
	if (isBlank(sid)) return;
	markDepartureDirty();
	setprop("/instrumentation/cdu/dep/showSelOnly",1);
	setprop("/autopilot/route-manager/departure/newsid", sid);
	setprop("/instrumentation/cdu/sids/sidIsSelected", 1);
	var newRwy = getRwyOfSids(sid);
	if (!isBlank(newRwy)){
		setprop("/autopilot/route-manager/departure/newrunway", newRwy);
		setprop("/instrumentation/cdu/sids/rwyIsSelected", 1);
	}
	setprop("/autopilot/route-manager/departure/sidID", sid);
}

var selectDepartureRunway = func(line){
	var rwy = getCduOutput(line, "right");
	if (isBlank(rwy)) return;
	var current = getprop("/autopilot/route-manager/departure/newrunway");
	if (current == nil) { current = ""; }
	if (rwy != current){
		clearDepartureSidSelection();
	}
	markDepartureDirty();
	setprop("/autopilot/route-manager/departure/newrunway", rwy);
	setprop("/instrumentation/cdu/sids/rwyIsSelected", 1);
}

var selectArrivalStar = func(line){
	if (getprop("/instrumentation/cdu/arr/isSelectable") != 1) return;
	var star = getCduOutput(line, "left");
	if (isBlank(star)) return;
	markArrivalDirty();
	setprop("/autopilot/route-manager/destination/newStar", star);
	setprop("/instrumentation/cdu/stars/starIsSelected", 1);
}

var selectArrivalApproach = func(line){
	if (getprop("/instrumentation/cdu/arr/isSelectable") != 1) return;
	var appr = getCduOutput(line, "right");
	if (isBlank(appr)) return;
	markArrivalDirty();
	setprop("/autopilot/route-manager/destination/newApproach", appr);
...
var key = func(v) {
		...
		var wpFromKey = func(k) {
			return "/autopilot/route-manager/route/wp[" ~ (k + ((crtPageNum - 1) * 5)) ~ "]/altitude-ft";
		}
		...
		if (serviceable == 1){
			...
			if (v == "LSK1R"){
				var fromDepArrIndex = (cduDisplay == "DEP_ARR_INDEX");
				if (cduDisplay == "DEP_ARR_INDEX"){
					var depAirport = getprop("/autopilot/route-manager/departure/airport");
					if (!isBlank(depAirport)){
						setArrivalContext(depAirport, 0);
						cduDisplay = "RTE1_ARR";
					}
				}
				if (cduDisplay == "RTE1_DEP"){
					selectDepartureRunway(1);
				}
				if (cduDisplay == "RTE1_ARR" and !fromDepArrIndex){
					selectArrivalApproach(1);
				}
				if (cduDisplay == "EICAS_MODES"){
					eicasDisplay = "FUEL";
				}
				...
				if (cduDisplay == "RTE1_LEGS"){
					var prop = wpFromKey(0);

					setprop(prop, cduInput);
					if (substr(cduInput,0,2) == "FL"){
						setprop(prop, substr(cduInput, 1) * 100);
					}
					cduInput = "";
				}
				if (cduDisplay == "POS_REF_0"){
			...
			if (v == "LSK2R"){
				if (cduDisplay == "RTE1_ARR"){
					selectArrivalApproach(2);
				}
				if (cduDisplay == "RTE1_DEP"){
					selectDepartureRunway(2);
				}else if (cduDisplay == "DEP_ARR_INDEX")
				{
					var destAirport = getprop("/autopilot/route-manager/destination/airport");
					if (!isBlank(destAirport)){
						setArrivalContext(destAirport, 1);
						cduDisplay = "RTE1_ARR";
					}
				}
					...
					var prop = wpFromKey(1);
					setprop(prop, cduInput);
					if (substr(cduInput,0,2) == "FL"){
						setprop(prop, substr(cduInput, 2) * 100);
					}
					cduInput = "";
				}
			...
			if (v == "LSK3R"){
				if (cduDisplay == "RTE1_ARR"){
					selectArrivalApproach(3);
				}
				if (cduDisplay == "RTE1_DEP"){
					selectDepartureRunway(3);
					}
				if (cduDisplay == "NAV_RAD"){
					if (int(cduInput) > 189 and int(cduInput) < 1751) {
			...
			if (v == "LSK5R"){
				if (cduDisplay == "RTE1_ARR"){
					selectArrivalApproach(5);
				}
				if (cduDisplay == "RTE1_DEP"){
					selectDepartureRunway(5);
					}
				if (cduDisplay == "RTE1_LEGS"){
					var prop = wpFromKey(4);
					setprop(prop, cduInput);
					if (substr(cduInput,0,2) == "FL"){
						setprop(prop, substr(cduInput, 2) * 100);
					}
			...
			if (v == "LSK6L"){
				...
				if (cduDisplay == "INIT_REF"){
					...
					if (arrDirty == nil) { arrDirty = 0; }
					if(arrDirty != 0){
						var activeAppr = getprop("/autopilot/route-manager/destination/approach");
						if (!isBlank(activeAppr)){
							setprop("/autopilot/route-manager/destination/newApproach", activeAppr);
							setprop("/instrumentation/cdu/appr/apprIsSelected", 1);
						}else{
							setprop("/autopilot/route-manager/destination/newApproach", "");
							setprop("/instrumentation/cdu/appr/apprIsSelected", 0);
						}
						var activeStar = getprop("/autopilot/route-manager/destination/star");
						if (!isBlank(activeStar)){
							setprop("/autopilot/route-manager/destination/newStar", activeStar);
							setprop("/instrumentation/cdu/stars/starIsSelected", 1);
						}else{
							setprop("/autopilot/route-manager/destination/newStar", "");
							setprop("/instrumentation/cdu/stars/starIsSelected", 0);
						}
						setprop("/instrumentation/cdu/appr/page", 1);
						clearArrivalDirty();
					...
					var depDirty = getprop("/instrumentation/cdu/dep/isChanged");
					if (depDirty == nil) { depDirty = 0; }
					if(depDirty != 0){
...
var cduUpdate = func{
		...
		var cduDisplayRefresh = func(){
			...
			cduProps.setValue("line5/center-bar", line5cBar);
			cduProps.setValue("line6/center-bar", line6cBar);
		}
		...
		if (display == "POS_INIT") {
			...
			if (getprop("/instrumentation/fmc/isInputedPos") == 1){
				...
				# Still needs to adjust font for best results
				line5r = "___*__._ ____*__._";
			}
		...
		if (display == "RTE1_ARR") {
			...
			var arrAirport = getArrAirport();
			if (!isBlank(arrAirport)){
				title = arrAirport ~ " ARRIVALS";
			}else{
				title = "ARRIVALS";
			}
			var arrDirty = getprop("/instrumentation/cdu/arr/isChanged");
			...
			var allowAutoSelect = (getprop("/instrumentation/cdu/arr/isSelectable") == 1);
			var showSelOnly = (arrDirty != 0 and getprop("/instrumentation/cdu/arr/showSelOnly") == 1);
			line1ctl = "RTE 1";
			line1lt = "STARS";
			line1rt = "APPROACHES";
			if (!allowAutoSelect){
			...
			var activeStar = getprop("/autopilot/route-manager/destination/star");
			var pendingStar = getprop("/autopilot/route-manager/destination/newStar");
			var selStar = (arrDirty != 0) ? pendingStar : activeStar;
			if (arrDirty == 0 and isBlank(selStar)){
				selStar = activeStar;
			}
			var starSelected = (!isBlank(selStar) and getprop("/instrumentation/cdu/stars/starIsSelected") == 1);
			if (arrDirty == 0 and allowAutoSelect and !isBlank(selStar)){
				starSelected = 1;
			}
			if (showSelOnly and starSelected and allowAutoSelect){
				line1cl = selOrAct;
				line1l = trimProcRunwaySuffix(selStar);
				line2l = "";
				line3l = "";
				line4l = "";
				line5l = "";
			}else{
				var starList = echoStars(getprop("/instrumentation/cdu/appr/page"));
				line1l = starList[0];
				line2l = starList[1];
				line3l = starList[2];
				line4l = starList[3];
				line5l = starList[4];
				if (allowAutoSelect and !isBlank(selStar)){
					if (line1l == selStar) { line1cl = selOrAct; line1l = trimProcRunwaySuffix(line1l); }
					else if (line2l == selStar) { line2cl = selOrAct; line2l = trimProcRunwaySuffix(line2l); }
					else if (line3l == selStar) { line3cl = selOrAct; line3l = trimProcRunwaySuffix(line3l); }
					else if (line4l == selStar) { line4cl = selOrAct; line4l = trimProcRunwaySuffix(line4l); }
					else if (line5l == selStar) { line5cl = selOrAct; line5l = trimProcRunwaySuffix(line5l); }
				}
			}

			var activeAppr = getprop("/autopilot/route-manager/destination/approach");
			var pendingAppr = getprop("/autopilot/route-manager/destination/newApproach");
			var selAppr = (arrDirty != 0) ? pendingAppr : activeAppr;
			if (arrDirty == 0 and isBlank(selAppr)){
				selAppr = activeAppr;
			}
			var apprSelected = (!isBlank(selAppr) and getprop("/instrumentation/cdu/appr/apprIsSelected") == 1);
			if (arrDirty == 0 and allowAutoSelect and !isBlank(selAppr)){
				apprSelected = 1;
			}
			if (showSelOnly and apprSelected and allowAutoSelect){
				line1r = selAppr;
				line1cr = selOrAct;
				line2rt = "TRANS";
				line2r = "WIP";# WORK IN PROGRESS
			}else{
				var apprList = echoAppr(getprop("/instrumentation/cdu/appr/page"));
				line1r = apprList[0];
				line2r = apprList[1];
				line3r = apprList[2];
				line4r = apprList[3];
				line5r = apprList[4];
				if (allowAutoSelect and !isBlank(selAppr)){
					if (line1r == selAppr) line1cr = selOrAct;
					else if (line2r == selAppr) line2cr = selOrAct;
					else if (line3r == selAppr) line3cr = selOrAct;
					else if (line4r == selAppr) line4cr = selOrAct;
					else if (line5r == selAppr) line5cr = selOrAct;
				}
			}

			if(getprop("/instrumentation/cdu/appr/apprCountEnd") != 0 and !(showSelOnly and apprSelected and allowAutoSelect)){
		...
		if (display == "RTE1_DEP") {
			...
			var pendingSid = getprop("/autopilot/route-manager/departure/newsid");
			var selSid = (depDirty != 0) ? pendingSid : activeSid;
			if (depDirty == 0 and isBlank(selSid)){
				selSid = activeSid;
			}
			var showSelOnly = (depDirty != 0 and getprop("/instrumentation/cdu/dep/showSelOnly") == 1);
			var sidSelected = !isBlank(selSid);
			if (showSelOnly and sidSelected){
				line1cl = selOrAct;
				line1l = selSid;
				line2lt = "TRANS";
				line2l = "WIP";
				...
				if(getprop("/instrumentation/cdu/sids/rwyIsSelected") == 0){
					sidList = echoSids(getprop("/instrumentation/cdu/sids/page"));
				}else{
					sidList = echoSids(getprop("/instrumentation/cdu/sids/page"), getprop("/autopilot/route-manager/departure/newrunway"));
				}
				line1l = sidList[0];
				line2l = sidList[1];
				line3l = sidList[2];
				...
				line5l = sidList[4];
				if (!isBlank(selSid)){
					if (line1l == selSid) line1cl = selOrAct;
					else if (line2l == selSid) line2cl = selOrAct;
					else if (line3l == selSid) line3cl = selOrAct;
					else if (line4l == selSid) line4cl = selOrAct;
					else if (line5l == selSid) line5cl = selOrAct;
				}
			}

			var newRunway = getprop("/autopilot/route-manager/departure/newrunway");
			if (isBlank(newRunway)){
				setprop("/instrumentation/cdu/sids/rwyIsSelected", 0);
				if (isBlank(pendingSid)){
					setprop("/instrumentation/cdu/sids/sidIsSelected", 0);
					setprop("/autopilot/route-manager/departure/sidID", "");
					setprop("/autopilot/route-manager/departure/newsid", "");
				}
			...
			if(getprop("/instrumentation/cdu/sids/rwyIsSelected") == 0){
				var rwyList = echoRwys(getprop("/instrumentation/cdu/sids/page"));
				line1r = rwyList[0];
				line2r = rwyList[1];
				line3r = rwyList[2];
				line4r = rwyList[3];
			...
			var selRunway = (depDirty != 0) ? newRunway : getprop("/autopilot/route-manager/departure/runway");
			if (depDirty == 0 and isBlank(selRunway)){
				selRunway = newRunway;
			}
			if (!isBlank(selRunway)){
				if (line1r == selRunway) line1cr = selOrAct;
				else if (line2r == selRunway) line2cr = selOrAct;
				else if (line3r == selRunway) line3cr = selOrAct;
				else if (line4r == selRunway) line4cr = selOrAct;
				else if (line5r == selRunway) line5cr = selOrAct;
			}
			line6cBar = displayBar;
			line1rt = "RUNWAYS";
			#if (getprop("/autopilot/route-manager/departure/newrunway") != nil){
		...
		if (display == "RTE1_LEGS") {
			...
			page = sprintf("%d/%d", crtPageNum, pageNum);
...
var cduTimer = maketimer(0.2, cduUpdate);
...
#_setlistener("/sim/signals/fdm-initialized", cduUpdate);
